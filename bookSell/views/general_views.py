...
def book(request, book_id):
    ...
    if headers[sort] == "des":
        ...
        headers[sort] = "des"
    return render(request, 'books/book_view.html', {'book': book, 'books_for_sale': books_for_sale, 'genres' : genres})

def search(request):
    query = request.GET['q']
    results = Book.objects.filter(Q(title__icontains=query) | Q(author__icontains=query))
    return render(request, 'books/search_results.html', {'query':query, 'books':results, 'genres' : genres})

def genre(request, genre):
    return render(request, 'books/genrePage/genre_page.html', {'genre': genre, 'genres' : genres})
